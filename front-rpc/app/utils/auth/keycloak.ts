...
export class KeycloakStrategy<User> extends OAuth2Strategy<User> {
  ...
  constructor(
    ...
    verify: Strategy.VerifyFunction<User, any>
  ) {
   ...
   */
  protected authorizationParams(urlSeachParams: URLSearchParams): URLSearchParams {
    urlSeachParams.set("scope", this.scope);
